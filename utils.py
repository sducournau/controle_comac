...
def reload_config(data, PRECEDENT_GESTIONNAIRE):
    ...
    global CONNEXION
    try:
        CONNECTION = psycopg2.connect(config_data['CONNEXION'])
        iface.messageBar().pushMessage("Success", "La connexion BDD a bien été établie.",level=Qgis.Success, duration=5)
    except:
        CONNECTION = None
        iface.messageBar().pushMessage("Error", "La connexion BDD n'a pas pu être établie.",level=Qgis.Critical, duration=5)
...
class populateComboBox:
    ...
    def populate_eplan(self):



        self.dockwidget.comboBox_convention.clear()
        self.dockwidget.comboBox_convention.addItems([x for x in eplan_convention_list.keys()])

        self.dockwidget.comboBox_dr.clear()
        self.dockwidget.comboBox_dr.addItems([x for x in eplan_dr_list.keys()])
